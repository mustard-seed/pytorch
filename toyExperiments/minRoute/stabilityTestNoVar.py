'''
stabilityTestNoVar.py
Purpose: 
    Study the stability of the minimization algorith at the optimal soluution with fixed distribution variance
'''
import torch
...
def main():
    ...
    for index, m in enumerate(listM):
        outputDirectory = os.path.join(os.path.dirname(__file__),'stabilityNoVarOutputs')
        ...
        varSigma2Initial = torch.tensor(np.ones(points.shape), dtype=torch.float)
        ...
        records = routeModel.minimize_route(numRun = numRun, \
            ...
            requiresMuGrad = True, requiresSigmaGrad = False)
        ...
        ax.set_title( ("Optimize mu only \n" + currTime+" m = {m}, n = {n}, s = {s}, lr={lr}, beta1={beta1}, beta2={beta2}") \
            .format(m=m, n=n, s = s, lr=learning_rate, beta1=beta1, beta2=beta2))
        ax.set_xlabel("Iterations", fontsize=12)
        ax.set_ylabel("Loss", fontsize=12)
        plt.savefig(fname="stabilityNoVarOutputs/"+currTime+".png", format='png')
